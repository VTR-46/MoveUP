...
function buildSummary() {
  ...
  let html = '';
  if (user.nome) {
    html += `<div class="flex items-start gap-2"><span class="text-[11px] text-gray-400 w-20 shrink-0 pt-0.5">Nome</span><span class="text-[13px] font-medium text-dark">${user.nome} ${user.sobrenome || ''}</span></div>`;
  }
  if (nivel) {
